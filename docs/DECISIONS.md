...
## ADR-007: Real 3D board (three.js + provided .glb assets), superseding ADR-005

**Phase:** A8 follow-up (explicit user request: "I want you to use them" — the board/checker/dice assets)
**Spec reference:** §11 (art assets, asset manifest), §14 A8

**Decision:** The match board is now rendered with three.js using the actual provided `.glb` assets — board, checker set, and dice, each independently selectable per player via a lobby "Table settings" panel with a live 3D preview, persisted in `localStorage`. This directly reverses ADR-005's earlier decision to keep the interactive surface 2D SVG.

**What changed technically, and why it's trustworthy, not guessed:**
- The asset `.glb` files were inspected directly (parsing the glTF JSON chunk) rather than assumed. Every board theme has `point_1`..`point_24` nodes with real baked transform matrices — cross-checked against the fullset's embedded starting-position preview (`chk_*` nodes), which matches the engine's exact starting layout. This confirms `point_N` in the asset maps 1:1 to the engine's point numbering with no relabeling needed, and that checker/point positions are exact, not approximated.
- **Per-player orientation is a world-mirror, not a camera move.** Both home boards sit on the same physical x-side of this asset (split by z only) — relocating the camera to "the other end of the table" for Black flips left/right along with front/back (a real bug caught and fixed mid-session: Black's home landed bottom-left instead of bottom-right). The camera is now fixed and identical for both players; a wrapper `worldGroup` mirrors on z only for Black's view. All materials are forced double-sided since a mirror transform flips triangle winding.
- **Camera framing adapts to the container's live aspect ratio** rather than using fixed numbers. It's recomputed via `_positionCamera()` (called on every resize, with `devicePixelRatio` also re-read then, since moving windows between monitors doesn't reliably fire a resize on its own): the camera holds a fixed viewing angle and iteratively solves for the distance that keeps four known board corners within a safety margin of the visible frame. Two earlier attempts at a fixed distance either left too much empty page space or clipped the bottom corners on narrower windows (a laptop vs. a wide monitor) — this replaced both.
- **Bear-off has no dedicated geometry in the asset set** — built as simple marker trays just past the board edge, aligned with each color's home row; missed in the first pass (would have made bearing off unclickable) and added after review.
- **Dice show their rolled value via a numeral sprite**, not true pip-face physical orientation — decoding the asset's baked per-face rotation data with confidence wasn't practical in the time available; the cup-shake/dice-settle animation is real, the exact resting orientation is not physically simulated.
- **Only one dice-cup model exists in the asset set**, not one per board theme — flagged before building; every theme shares the single cup rather than a fabricated "Persian thrower."

**Verified, not assumed:** live two-browser sessions at multiple distinct viewport aspect ratios (1024×768, 1280×800, 1400×900, 1440×900) with zero console errors; click-to-move confirmed via real raycasts (not simulated events); a full turn-by-turn game played through actual clicks on the rendered canvas; and orientation correctness proven by projecting each player's actual home-point world coordinates through their own camera/world matrix and reading the resulting screen-space position directly (not by eyeballing screenshots, which had already produced one false read during this work).

## ADR-008: Hit checkers weren't visible on the bar; stacks over 5 need layering, not just a "+N" label

**Phase:** A8 follow-up (3D board polish, user report)
**Spec reference:** §4.1 (bar/hit mechanics)

**Decision / fixes:**
1. **Bar checkers now sit on the bar's real measured top surface**, not its center pivot. `barWorldPos` (the bar node's origin) sat roughly mid-height through the bar's own geometry, so a checker placed at `barWorldPos.y + halfHeight` was partially embedded inside the bar mesh rather than resting visibly on top of it — a hit checker was technically rendered but not visible. Fixed by measuring the bar's actual world-space bounding box (`new THREE.Box3().setFromObject(barNode).max.y`) once at theme load and placing checkers relative to that measured top, not a guessed offset.
2. **Checkers now stack in layers (5 per row, up to 3 layers = 15) instead of capping display at 5 with a "+N" label immediately.** The "+N" overflow label still appears, now only beyond 15 (the realistic max for one point), matching how a real board actually piles extra checkers on top rather than hiding them behind a counter. Applied to both regular points and the bar.

**Verified:** live gameplay confirmed the app renders correctly during normal play; the two specific conditions being fixed (a hit, and a stack past 5) are rare/slow to hit via random play in a live game, so they were verified precisely and deterministically instead — calling `board.render()` directly with synthetic game states (8 and 7 checkers on two points; 3 and 2 on each bar; a 17-checker bar overflow case) and confirming via screenshot and exact mesh-count assertions (`checkerGroup.children.length`) that: bar checkers render visibly on top of the bar, stacks up to 15 render as individual layered checkers, and the "+N" sprite appears exactly once a stack exceeds 15 (e.g. 17 -> 15 checkers + one "+2" sprite).

**Also worth noting:** this surfaced a testing-methodology bug worth recording — several earlier verification scripts in this session used DOM locators (`.board-root [data-point].selectable`) copied from the old 2D SVG board's test patterns. The 3D board is a `<canvas>` with no such DOM structure; those locators silently matched zero elements every time rather than erroring, producing a false "no interaction available" reading that was actually a broken test, not a broken app. Real 3D interaction tests must use pixel-coordinate mouse clicks against the canvas (as the orientation/camera-fit verifications in ADR-007 correctly did) or direct calls into the exposed board instance, never DOM element locators.

## ADR-009: Real physical dice pips, decoded from the asset's actual geometry

**Phase:** A8 follow-up (3D board polish, user report + explicit request)
**Spec reference:** §11 (art assets), user request to replace the numeral-label dice from ADR-007

**Decision:** Dice no longer show their rolled value via a floating text sprite (the ADR-007 simplification). Each `dice_*.glb` asset bakes real pip geometry onto all 6 faces of the die as a single rigid body (`die_1_cube` + ~21 `die_1_pip` sibling nodes under a `die_1` parent) — a real physical die, not one mesh per value. The face-to-value mapping was decoded directly from the asset's actual pip node positions (grouped by which local axis is the face normal, then by pip count per group: 1 pip on local +Y, 6 on -Y, 3 on +Z, 4 on -Z, 2 on +X, 5 on -X — a standard die, opposite faces summing to 7, confirmed identical across all four dice color themes). Showing a value is now just rotating that value's local axis to world-up via `THREE.Quaternion.setFromUnitVectors`, plus a random spin around the vertical axis for visual variety. The dice also now visibly tumble (slerp from a random start orientation to the correct resting rotation) during the throw animation instead of popping in already-settled.
